...
contract LzApp is Pausable, BaseNonblockingLzApp {
    event SendToChain(address indexed _sender, uint16 indexed _dstChainId, address indexed _toAddress, uint _amount, uint64 _nonce, bytes payload);
    event Mint(address indexed _toAddress, bytes indexed _dstChainId,  uint _amount, uint64 _nonce);
